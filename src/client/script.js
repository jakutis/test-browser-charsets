(function() {
  ...
  "x-mac-cyrillic":[1040,1041,1042,1043,1044,1045,1046,1047,1048,1049,1050,1051,1052,1053,1054,1055,1056,1057,1058,1059,1060,1061,1062,1063,1064,1065,1066,1067,1068,1069,1070,1071,8224,176,1168,163,167,8226,182,1030,174,169,8482,1026,1106,8800,1027,1107,8734,177,8804,8805,1110,181,1169,1032,1028,1108,1031,1111,1033,1113,1034,1114,1112,1029,172,8730,402,8776,8710,171,187,8230,160,1035,1115,1036,1116,1109,8211,8212,8220,8221,8216,8217,247,8222,1038,1118,1039,1119,8470,1025,1105,1103,1072,1073,1074,1075,1076,1077,1078,1079,1080,1081,1082,1083,1084,1085,1086,1087,1088,1089,1090,1091,1092,1093,1094,1095,1096,1097,1098,1099,1100,1101,1102,8364]
    ...
    var singleByteEncodedStringToBytes = function(index, str) {
        var code, bytes = [];
        for(var i = 0; i < str.length; i += 1) {
            code = str.charCodeAt(i);
            bytes.push(code < 128 ? code : indexOf(index, code) + 128);
        }
        return bytes;
    };
    ...
    var dataNames = [];
    for(var i in expectedBytes) {
        if(expectedBytes.hasOwnProperty(i)) {
            dataNames.push(i);
        }
    }
    var request = function(url, method, overrideMimeType, data, cb) {
        ...
        var onload = function() {
            ...
            if(typeof xhr.responseText !== 'string') {
                cb(new Error('responseText is not a string'));
        ...
        xhr.setRequestHeader('Accept', overrideMimeType);
        xhr.send(data);
    };
    ...
    var requestResultByUserAgent = function(userAgent, cb) {
        request('/result/by-userAgent/' + encodeURIComponent(userAgent), 'GET', 'application/json', null, function(err, output) {
            if(err) {
                return cb(err);
            }
    ...
    var getResults = function(cb) {
        request('/result', 'GET', 'application/json', null, function(err, output) {
            if(err) {
                return cb(err);
            }
            cb(null, JSON.parse(output));
    ...
    var postResults = function(result, cb) {
        request('/result', 'POST', 'application/json', JSON.stringify(result), function(err, output) {
            if(err) {
                return cb(err);
            }
            cb(null, JSON.parse(output));
    ...
    var displayStatus = function(status) {
        ...
        statusEl.replaceChild(document.createTextNode(status), statusEl.childNodes[0]);
    };
    var displayResults = function(results) {
        ...
        resultsEl.appendChild(theadEl);
        var headerEl = document.createElement('tr');
        theadEl.appendChild(headerEl);
        var userAgentHeaderEl = document.createElement('th');
        userAgentHeaderEl.appendChild(document.createTextNode('Browser ID'));
        theadEl.appendChild(userAgentHeaderEl);

        for(i in results[0].byTestData[dataNames[0]]) {
            if(results[0].byTestData[dataNames[0]].hasOwnProperty(i)) {
                var typeHeaderEl = document.createElement('th');
                typeHeaderEl.appendChild(document.createTextNode(i));
                theadEl.appendChild(typeHeaderEl);
            }
        }
        var tbodyEl = document.createElement('tbody');
        resultsEl.appendChild(tbodyEl);
        for(i = 0; i < results.length; i += 1) {
            var resultEl = document.createElement('tr');
            var userAgentEl = document.createElement('td');
            userAgentEl.style.whiteSpace = 'pre';
            userAgentEl.appendChild(document.createTextNode(results[i].userAgent));
            resultEl.appendChild(userAgentEl);
            for(var type in results[i].byTestData[dataNames[0]]) {
                if(results[i].byTestData[dataNames[0]].hasOwnProperty(type)) {
                    var failed = [];
                    for(j = 0; j < dataNames.length; j += 1) {
                        if(!results[i].byTestData[dataNames[j]][type]) {
                            failed.push(dataNames[j]);
                        }
                    }
                    var charsetEl = document.createElement('td');
                    charsetEl.appendChild(document.createTextNode(failed.toString()));
                    if(failed.length === 0) {
                        charsetEl.style.backgroundColor = 'green';
                    }
                    resultEl.appendChild(charsetEl);
                }
            }
            tbodyEl.appendChild(resultEl);
        }
        document.body.appendChild(resultsEl);
    };
    var requestAllTests = function(cb) {
        var dataI = 0;
        var results = {};
        var next = function() {
            if(dataI >= dataNames.length) {
                return postResults({
                    byTestData: results
                }, cb);
            }
            requestTests(dataNames[dataI], function(err, result) {
                if(err) {
                    return cb(err);
                }
                results[dataNames[dataI]] = result;
                dataI += 1;
                next();
            });
        };
    ...
    requestResultByUserAgent(navigator.userAgent, function(err, result) {
        if(err) {
            return requestAllTests(function(err, results) {
                if(err) {
                    return displayStatus('error occurred: ' + err);
                }
